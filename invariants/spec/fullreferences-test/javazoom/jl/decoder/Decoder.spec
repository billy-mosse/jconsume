...
<spec>
	<class decl="javazoom.jl.decoder.Decoder">   
    	<method decl="javazoom.jl.decoder.Obuffer decodeFrame(javazoom.jl.decoder.Header,javazoom.jl.decoder.Bitstream,javazoom.jl.decoder.FrameDecoder)">
			<relevant-parameters>header.num_subbands,header.mode,header.h_intensity_stereo_bound</relevant-parameters>
			<requires>header.num_subbands >= 0 and header.mode >= 0 and header.mode &lt; 4 and header.h_intensity_stereo_bound >= 0 and header.num_subbands >= header.h_intensity_stereo_bound</requires>
			<call-site offset="1">
				<constraints>header.num_subbands == $t.this.num_subbands and $t.this.mode == header.mode and header.h_intensity_stereo_bound == $t.this.header.h_intensity_stereo_bound</constraints>
			</call-site>
		</method>
