...
<spec>
  <class decl="javazoom.jl.decoder.LayerIDecoder">
    <method decl="void decodeFrame()">
      <relevant-parameters>layer,this.header.h_intensity_stereo_bound,this.mode,this.num_subbands</relevant-parameters>
      <requires>this.mode >= 0 and this.num_subbands >= 0 and this.header.h_intensity_stereo_bound >= 0 and this.num_subbands >=  this.header.h_intensity_stereo_bound and layer > 0 and layer &lt; 3 and this.mode &lt; 4</requires>
      <call-site offset="2" impl="javazoom.jl.decoder.LayerIDecoder">
        <constraints><![CDATA[layer == 1 and this.num_subbands == $t.this.num_subbands and this.mode == $t.this.mode and this.header.h_intensity_stereo_bound == $t.this.header.h_intensity_stereo_bound]]></constraints>
      </call-site>
      <call-site offset="2" impl="javazoom.jl.decoder.LayerIIDecoder">
        <constraints><![CDATA[layer == 2 and this.num_subbands == $t.this.num_subbands and this.mode == $t.this.mode and this.header.h_intensity_stereo_bound == $t.this.header.h_intensity_stereo_bound]]></constraints>
      </call-site>
