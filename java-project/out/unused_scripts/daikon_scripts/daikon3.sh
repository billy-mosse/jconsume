...
#java -cp $CLASSPATH:$DAIKON_JAR daikon.DynComp "--ppt-select-pattern=InstrumentedMethod" $1 $2

echo "Chicory"

java -cp $DAIKON_JAR::$CLASSPATH daikon.Chicory "--output-dir=./daikon-output" --dtrace-file=$1.dtrace.gz "--ppt-select-pattern=InstrumentedMethod"  $1 $2

echo Ejecutando Daikon

java -cp $DAIKON_JAR::$CLASSPATH daikon.Daikon ./daikon-output/$1.dtrace.gz "--ppt-select-pattern=InstrumentedMethod.*:::ENTER" --nohierarchy --config_option daikon.Daikon.enable_floats=false --config_option daikon.inv.binary.twoScalar.FloatEqual.enabled=false --config_option daikon.inv.binary.twoScalar.LinearBinaryFloat.enabled=false --config_option  daikon.inv.ternary.threeScalar.LinearTernaryFloat.enabled=false --config_option  daikon.inv.unary.scalar.Modulus.enabled=true --config_option  daikon.inv.unary.scalar.NonModulus.enabled=true -o ./daikon-output/$1.inv.gz
