...
java -cp .:$CLASSPATH:$DAIKON_JAR daikon.Chicory \
     --comparability-file=$2.decls-DynComp \
     --premain=$CHICORY "--output-dir=./daikon-output" --dtrace-file=$1$2.dtrace.gz "--ppt-select-pattern=InstrumentedMethod"  $1$2 $3

echo "Daikon"

java -cp .:$CLASSPATH:$DAIKON_JAR daikon.Daikon $2.dtrace.gz "--ppt-select-pattern=InstrumentedMethod.*:::ENTER" --nohierarchy --config_option daikon.Daikon.enable_floats=false --config_option daikon.inv.binary.twoScalar.FloatEqual.enabled=false --config_option daikon.inv.binary.twoScalar.LinearBinaryFloat.enabled=false --config_option  daikon.inv.ternary.threeScalar.LinearTernaryFloat.enabled=false --config_option  daikon.inv.unary.scalar.Modulus.enabled=true --config_option  daikon.inv.unary.scalar.NonModulus.enabled=true -o ./daikon-output/$1$2.inv.gz
