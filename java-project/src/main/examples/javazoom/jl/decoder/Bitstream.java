...
public final class Bitstream implements BitstreamErrors
{
  ...
  //private int				last_frame_number;
	...
	/**
	 * FIXME El consumo depende del size que viene en el header
	 * Load ID3v2 frames.
	...
	private void loadID3v2(InputStream in) {		
		...
		try
		{
			...
			//System.out.println("loadID3v2.size=" + size );
			header_pos = size;			
		}
		catch (IOException e)
		{}
	...
	public Header readFrame() throws BitstreamException {
		Header result = null;
		try {
			result = readNextFrame(); //Residual de 1, el CRC pero deberia ser parametrico en el estado.
			// E.B, Parse VBR (if any) first frame.
			if (firstframe == true) {
				result.parseVBR(frame_bytes); //Residual de 7 en parseVBR
				firstframe = false;
			}			
			...
			if ((ex.getErrorCode()==INVALIDFRAME)) {
				...
				try {
					closeFrame();
					result = readNextFrame(); //Aca hay otro residual de 1
