...
class LayerIIDecoder extends LayerIDecoder implements FrameDecoder {
	...
	/* (non-Javadoc)
	 * @see javazoom.jl.decoder.LayerIDecoder#createSubbands()
	 */
	/**
	 * Temporal = 0
	 * Residual = this.num_subbands
	 */
	protected void createSubbands(){		
  		int i;
		if (mode == Header.SINGLE_CHANNEL) {
			//System.out.println("mode= single,num_subbands="+ num_subbands);
  		  for (i = 0; i < num_subbands; ++i)
  		    subbands[i] = new SubbandLayer2(i);
		}else if (mode == Header.JOINT_STEREO) {
			//System.out.println("mode=stereo,intensity="+header.intensity_stereo_bound()+"num_subbands=" + num_subbands);
  		  for (i = 0; i < header.intensity_stereo_bound(); ++i)
  		      subbands[i] = new SubbandLayer2Stereo(i);
  		  for (; i < num_subbands; ++i)
  		      subbands[i] = new SubbandLayer2IntensityStereo(i);
  		} else 	{
  			//System.out.println("mode=" + mode + ",num_subbands="+ num_subbands);
  		  for (i = 0; i < num_subbands; ++i)
  		      subbands[i] = new SubbandLayer2Stereo(i);
        }
