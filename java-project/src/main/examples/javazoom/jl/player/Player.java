...
public class Player {	  	
	...
	/**
	 * 2 + Res(Bitstream.init<>) + Res(Decoder.init<>)
	 * Temporal 
	 * @param stream
	 * @param device
	...
	//FIXME agregue el boolean
	public boolean play() throws JavaLayerException {
		boolean play = play(Integer.MAX_VALUE);
		return play;
	}
	...
	protected boolean decodeFrame() throws JavaLayerException {		
		...
		try {
			...
			if (!this.decoder.initialized) {
				this.decoder.initialize(h);
				//System.out.println("LAYER: " + h.layer());
				//System.out.println("Subbands: " + h.number_of_subbands());
				//System.out.println("Mode: " + h.mode());
			}
			
			int layer = h.layer();
			
			FrameDecoder frameDecoder = this.decoder.retrieveDecoder(h, bitstream, layer);
			
			//sample buffer set when decoder constructed
			SampleBuffer output = (SampleBuffer) this.decoder.decodeFrame(h, bitstream, frameDecoder);
