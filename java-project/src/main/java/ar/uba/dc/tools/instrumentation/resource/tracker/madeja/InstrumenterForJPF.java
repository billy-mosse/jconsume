...
public class InstrumenterForJPF  extends BodyTransformer { 
	
		private static boolean countAfterReturn = false;
		
		private static Log log = LogFactory.getLog(InstrumenterForJPF.class);
		
		//public static final String LISTENER_PACKAGE = "ar.uba.dc.tools.instrumentation.resource.tracker.runtime";
		public static final String LISTENER_PACKAGE = "monitor";
		public  static final String LISTENER_CLASS =  LISTENER_PACKAGE + ".BooleanTracker";
		
		private static final String NEW_OBJECT_SIGNATURE = "void track()";
		private static final String MAIN_METHOD_SIGNATURE = "void main(java.lang.String[])";
		private static final String CALL_SIGNATURE = "void start()";
		private static final String CAUGHT_EXCEPTION_SIGNATURE = "void caughtExceptionEvent(java.lang.String)";		
		private static final String END_CALL_SIGNATURE = "void stop()";
		private static final String ENTER_PROGRAM_METHOD_SIGNATURE = "void start()";
		private static final String EXIT_PROGRAM_METHOD_SIGNATURE = "void stop()";
		...
		public InstrumenterForJPF(String mainClass, LifeTimeSummaryRepository repository) {
		...
		private void processStmt(Body body, Chain units, Stmt s, EscapeAnalysisResult result) {
			...
			if (s instanceof AssignStmt) {
				...
				if (exp instanceof AnyNewExpr) {
					...
					units.insertBefore(this.addNewObjectEvent(),s);					
				}
		...
		private List<Unit> addCallEvent(SootMethod method, Body body, Stmt s, String invokeStmt) {
			...
			addNewStaticInvokeExpr(code, CALL_SIGNATURE);
			return code;
		}
		...
		private List<Unit> addEndCallEvent(SootMethod method, Body body, Stmt s, String invokeStmt) {
			...
			addNewStaticInvokeExpr(code, END_CALL_SIGNATURE);
	...
	//		addNewStaticInvokeExpr(code, NEW_OBJECT_SIGNATURE, localVar, StringConstant.v(insSite));
			addNewStaticInvokeExpr(code, NEW_OBJECT_SIGNATURE);
			return code;
		}
		private List<Unit> addNewObjectEvent() {
			List<Unit> code = new Vector<Unit>();
			addNewStaticInvokeExpr(code, NEW_OBJECT_SIGNATURE);
			return code;
		}
	...
	//	Scene.v().addBasicClass(LISTENER_CLASS,2);
