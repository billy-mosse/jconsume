...
public class BarvinokCalculatorWithLazyAsMaxStrategyTest {
	...
	public static CommandLineCalculator[] calculators() {
		...
		BarvinokSyntax[] syntaxes = new BarvinokSyntax[] { new IsccSyntax() };
		...
		for (BarvinokSyntax syntax : syntaxes) {
			...
			calc.setCompareStrategy(new LazyAsCompareStrategy());
	...
	public void upperBoundOperator(CommandLineCalculator calc) {
		...
		assertThat(m.getPieces().get(2).getPolynomial(), is(equalTo("max(m*n, m^2, n^2)")));
	...
	@Theory
	public void upperBoundOperatorSupportMaximum(CommandLineCalculator calc) {
		// Elimina candidatos si puede
		DomainSet d = new DomainSet("k == $t.n and j == $t.m and k > j and j > 0");
		d.addParameter("k");
		d.addParameter("j");
		d.addVariable("$t.n");
		d.addVariable("$t.m");
		...
		p.addParameter("n");
		p.addParameter("m");
		p.add(new QuasiPolynomial("max(infty)", "m >= 1 + n"));
		p.add(new QuasiPolynomial("max(n * m, m^2, n^2)", "m <= n"));
		PiecewiseQuasipolynomial m = calc.upperBound(p, d);
		assertThat(m.getParameters().size(), is(equalTo(2)));
		assertThat(m.getParameters(), hasItem("k"));
		assertThat(m.getParameters(), hasItem("j"));
		assertThat(m.getPieces().size(), is(equalTo(1)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("k >= 1 + j && j >= 1")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("max(j*k, j^2, k^2)")));
		
		// Si alguna parte tiene max, completa las que faltan
		d = new DomainSet("k == $t.n and j == $t.m and k > j and j > 0");
		d.addParameter("k");
		d.addParameter("j");
		d.addVariable("$t.n");
		d.addVariable("$t.m");
		...
		p.addParameter("n");
		p.addParameter("m");
		p.add(new QuasiPolynomial("infty", "m >= 1 + n"));
		p.add(new QuasiPolynomial("max(n * m, m^2, n^2)", "m <= n"));
		m = calc.upperBound(p, d);
		assertThat(m.getParameters().size(), is(equalTo(2)));
		assertThat(m.getParameters(), hasItem("k"));
		...
		d = new DomainSet("0 <= i <= k and 0 <= j < i and i == $t.n and j == $t.m");
		d.addParameter("k");
		d.addParameter("l");
		d.addVariable("$t.n");
		d.addVariable("$t.m");
		...
		m = calc.upperBound(p, d);
		assertThat(m.getParameters().size(), is(equalTo(2)));
		assertThat(m.getParameters(), hasItem("l"));
		assertThat(m.getParameters(), hasItem("k"));
		assertThat(m.getPieces().size(), is(equalTo(1)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("k >= 1")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("k^2")));
		
		// Genera nuevos candidatos si corresponde
		d = new DomainSet("l <= i <= k and l <= j < i and i == $t.n and j == $t.m and l >= 0");
		d.addParameter("k");
		d.addParameter("l");
		d.addVariable("$t.n");
		d.addVariable("$t.m");
		...
		assertThat(m.getPieces().size(), is(equalTo(1)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("l >= 0 && l <= -1 + k")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("max((-1/2)*k + (1/2)*k^2 + (1/2)*k*l, k + k*l, k^2, 1 + 2*l + l^2)")));
	}
	...
	@Theory
	public void addOperatorSupportMaximum(CommandLineCalculator calc) {
		PiecewiseQuasipolynomial p = new PiecewiseQuasipolynomial();
		p.addParameter("m");
		p.add(new QuasiPolynomial("max((-1 + 2 * m + m^2), (3 + m^2))", "m >= 1"));
		p.add(new QuasiPolynomial("max(m^2)", "m <= 0"));
		
		PiecewiseQuasipolynomial m = calc.add(p, BarvinokFactory.polynomial(p.getParameters(), new QuasiPolynomial("m*m")));
		assertThat(m.getParameters().size(), is(equalTo(1)));
		assertThat(m.getParameters(), hasItem("m"));
		assertThat(m.getPieces().size(), is(equalTo(2)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("m >= 1")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("max(-1 + 2*m + 2*m^2, 3 + 2*m^2)")));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("m <= 0")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("2*m^2")));
		
		m = calc.add(BarvinokFactory.polynomial(p.getParameters(), new QuasiPolynomial("m*m")), p);
		assertThat(m.getParameters().size(), is(equalTo(1)));
		assertThat(m.getParameters(), hasItem("m"));
		assertThat(m.getPieces().size(), is(equalTo(2)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		...
		p = new PiecewiseQuasipolynomial();
		p.addParameter("args.length");
		p.add(new QuasiPolynomial("23 + args.length", "args.length >= 1"));
		p.add(new QuasiPolynomial("23", "args.length <= 0"));
		
		PiecewiseQuasipolynomial q = new PiecewiseQuasipolynomial();
		q.addAllParameters(p.getParameters());
		q.add(new QuasiPolynomial("max(12, 1 + (1/2)*a + (1/2)*a^2)", "args.length >= 2"));
		q.add(new QuasiPolynomial("12", "args.length == 1"));
		q.add(new QuasiPolynomial("12", "args.length <= 0"));
		m = calc.add(p, BarvinokFactory.polynomial(p.getParameters(), new QuasiPolynomial("2")), q);
		assertThat(m.getParameters().size(), is(equalTo(1)));
		assertThat(m.getParameters(), hasItem("args.length"));
		assertThat(m.getPieces().size(), is(equalTo(3)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("args.length >= 2")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("max(37 + args.length, 26 + (3/2)*args.length + (1/2)*args.length^2)")));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("args.length == 1")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("37 + args.length")));
		assertThat(m.getPieces().get(2).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(2).getConstraints(), is(equalTo("args.length <= 0")));
		assertThat(m.getPieces().get(2).getPolynomial(), is(equalTo("37")));
	}
	...
	public void maxOperator(CommandLineCalculator calc) {
		...
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("n >= 0")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("14 + n")));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("n <= -1")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("14")));
		...
		// El maximo entre dos polinomios es el maximo
		p = new PiecewiseQuasipolynomial();
		p.addParameter("n");
		p.add(new QuasiPolynomial("n + 14", "n >= 0"));
		p.add(new QuasiPolynomial("n*n", "n < 0"));
		...
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("max(n^2, n)")));
		assertThat(m.getPieces().get(2).getVariables().size(), is(equalTo(0)));
		...
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("max(n, n^2)")));
		assertThat(m.getPieces().get(2).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(2).getConstraints(), is(equalTo("n <= -4")));
		assertThat(m.getPieces().get(2).getPolynomial(), is(equalTo("n^2")));
		
		
		// Compara bien cuando hay infinito
		p = new PiecewiseQuasipolynomial();
		p.addParameter("n");
		p.add(new QuasiPolynomial("n + 14", "n >= 0"));
		p.add(new QuasiPolynomial("n*n", "n < 0"));
	...
	@Theory
	public void maxOperatorSupportMaximum(CommandLineCalculator calc) {
		Set<String> params = new TreeSet<String>();
		params.add("$t.n");
		
		// Puedo darle como entrada candidatos y los resuelve
		PiecewiseQuasipolynomial m = calc.max(BarvinokFactory.constant(28L), BarvinokFactory.polynomial(params, new QuasiPolynomial("max($t.n, 3)", "$t.n >= -3")));
		assertThat(m.getParameters().size(), is(equalTo(1)));
		assertThat(m.getParameters(), hasItem("$t.n"));
		assertThat(m.getPieces().size(), is(equalTo(2)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("$t.n >= -3")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("max(28, $t.n)")));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("$t.n <= -4")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("28")));
		
		m = calc.max(BarvinokFactory.polynomial(params, new QuasiPolynomial("max($t.n, 3)", "$t.n >= -3")), BarvinokFactory.constant(28L));
		assertThat(m.getParameters().size(), is(equalTo(1)));
		assertThat(m.getParameters(), hasItem("$t.n"));
		assertThat(m.getPieces().size(), is(equalTo(2)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("$t.n >= -3")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo("max($t.n, 28)")));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("$t.n <= -4")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("28")));
		...
		p.addParameter("n");
		p.addParameter("m");
		p.add(new QuasiPolynomial(BarvinokSyntax.INFINITE, "m >= 1 + n"));
		p.add(new QuasiPolynomial("max(n * m, m^2, n^2)", "m <= n"));
		
		m = calc.max(p, BarvinokFactory.polynomial(p.getParameters(), new QuasiPolynomial("max(-2 + 2 * m, 3, 71*m)", "m >= 1")));
		assertThat(m.getParameters().size(), is(equalTo(2)));
		assertThat(m.getParameters(), hasItem("n"));
		assertThat(m.getParameters(), hasItem("m"));
		assertThat(m.getPieces().size(), is(equalTo(4)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("n <= -1 + m && m >= 1")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo(BarvinokSyntax.INFINITE)));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("n <= -1 + m && m <= 0")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo(BarvinokSyntax.INFINITE)));
		assertThat(m.getPieces().get(2).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(2).getConstraints(), is(equalTo("n >= m && m >= 1")));
		assertThat(m.getPieces().get(2).getPolynomial(), is(equalTo("max(m*n, m^2, n^2, 3, 71*m)")));
		assertThat(m.getPieces().get(3).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(3).getConstraints(), is(equalTo("n >= m && m <= 0")));
		assertThat(m.getPieces().get(3).getPolynomial(), is(equalTo("max(m*n, m^2, n^2)")));
		
		m = calc.max(BarvinokFactory.polynomial(p.getParameters(), new QuasiPolynomial("max(-2 + 2 * m, 3, 71*m)", "m >= 1")), p);
		assertThat(m.getParameters().size(), is(equalTo(2)));
		assertThat(m.getParameters(), hasItem("n"));
		assertThat(m.getParameters(), hasItem("m"));
		assertThat(m.getPieces().size(), is(equalTo(4)));
		assertThat(m.getPieces().get(0).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(0).getConstraints(), is(equalTo("m >= 1 && n <= -1 + m")));
		assertThat(m.getPieces().get(0).getPolynomial(), is(equalTo(BarvinokSyntax.INFINITE)));
		assertThat(m.getPieces().get(1).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(1).getConstraints(), is(equalTo("m >= 1 && n >= m")));
		assertThat(m.getPieces().get(1).getPolynomial(), is(equalTo("max(71*m, 3, m*n, m^2, n^2)")));
		assertThat(m.getPieces().get(2).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(2).getConstraints(), is(equalTo("n <= -1 + m && m <= 0")));
		assertThat(m.getPieces().get(2).getPolynomial(), is(equalTo(BarvinokSyntax.INFINITE)));
		assertThat(m.getPieces().get(3).getVariables().size(), is(equalTo(0)));
		assertThat(m.getPieces().get(3).getConstraints(), is(equalTo("n >= m && m <= 0")));
		assertThat(m.getPieces().get(3).getPolynomial(), is(equalTo("max(m*n, m^2, n^2)")));
	}
